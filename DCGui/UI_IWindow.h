...
#ifndef DCGUI_UI_IWINDOW_H
#define DCGUI_UI_IWINDOW_H
...
namespace DCGui
{
	class UI_IWindow : public QObject
	{
		Q_OBJECT
	public:
		//变量用来存储创建的所有menu对象
		...
		void CreateMenuBySqlite()
		{
			...
			//从配置文件中获取菜单并创建
			MenuVec mvec = m_ConfigParser->GetMenuVec();
			...
			for (int i = 0; i != menuNum; ++i)
			{
				...
				if (!mvec[i].toolbar.simplified().isEmpty())
				{
					...
					int toolPos = 4;
					if (mvec[i].toolbarPos > 0)
					{
						toolPos = mvec[i].toolbarPos;
					}
					pMainwindow->addToolBar(Qt::ToolBarArea(toolPos), currentToolbar);
				}

				//为当前菜单添加动作，根据获取的classID到action表中获取所有的动作并添加
				MenuActionVec actionVec = m_ConfigParser->GetMenuActionVec(mvec[i].classID);
