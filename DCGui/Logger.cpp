...
void Logger::CreateNewLogFile(const QString logFile /* = LOG_NAME */)
{
	...
	if (file.open(QIODevice::WriteOnly | QIODevice::Append))
	{
		...
		out << "*********************************************************************\n"
			<< QString("**                %1 - Logfile               **").arg("北京鼎创力维科技有限公司") << Qt::endl
			<< QString("**                 %1                          ").arg(QDateTime::currentDateTime().toString()) << Qt::endl
			<< "*********************************************************************\n"
			<< "Version: " << version << " " << vstate << Qt::endl << Qt::endl;
...
void Logger::Writeln(const QString aValue)
{
	...
	if (file.open(QIODevice::WriteOnly | QIODevice::Append))
	{
		...
		out << aValue << Qt::endl
			<< "--------------------------------------- "
			<< QTime::currentTime().toString() << Qt::endl;
		file.flush();
	}
