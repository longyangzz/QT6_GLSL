...
# linker.
function(__qt56_internal_merge_libs out_libs_var)
    foreach(dep ${ARGN})
...
function(__qt56_internal_get_dict_key_values out_var target_infix dict_name dict_key)
...
# Assumes dict_name to be an existing INTERFACE target.
function(__qt56_internal_memoize_values_in_dict target dict_name dict_key values)
    ...
    # Scenario in the context of __qt56_internal_walk_libs:
    ...
    get_target_property(versionless_alias "${target}" "_qt56_versionless_alias")
    if(versionless_alias)
        __qt56_internal_get_dict_key_values(
    ...
    get_target_property(versionfull_alias "${target}" "_qt56_versionfull_alias")
...
#
#
function(__qt56_internal_walk_libs
    ...
    __qt56_internal_get_dict_key_values(libs "${target}" "${dict_name}" "libs")
    __qt56_internal_get_dict_key_values(rcc_objects "${target}" "${dict_name}" "rcc_objects")
    ...
    if(libs MATCHES "-NOTFOUND$")
        ...
        # build purposes.
        get_target_property(main_target_rcc_objects ${target} _qt56_rcc_objects)
        if(main_target_rcc_objects)
            __qt56_internal_merge_libs(rcc_objects ${main_target_rcc_objects})
        endif()
        ...
        foreach(lib ${target_libs})
            ...
            __qt56_internal_strip_target_directory_scope_token("${lib}" lib)
            ...
            elseif(TARGET ${lib_target})
                if(NOT "${lib_target}" MATCHES "^(Qt|${QT_CMAKE_EXPORT_NAMESPACE})::.+")
                    ...
                    set(versioned_qt56_target "${QT_CMAKE_EXPORT_NAMESPACE}::${lib_target}")
                    if(TARGET "${versioned_qt56_target}")
                        set(lib_target ${versioned_qt56_target})
                    endif()
                endif()
                ...
                if(lib_target_type STREQUAL "INTERFACE_LIBRARY")
                    if(NOT ${direct})
                        ...
                        if(lib_libs_${target})
                            __qt56_internal_merge_libs(libs ${lib_libs_${target}})
                            set(is_module 0)
                        endif()
                        if(lib_rcc_objects_${target})
                            __qt56_internal_merge_libs(rcc_objects ${lib_rcc_objects_${target}})
                        endif()
                    else()
                        __qt56_internal_merge_libs(libs ${lib})
                    endif()
                elseif(NOT lib_target_type STREQUAL "OBJECT_LIBRARY")

                    if(operation MATCHES "^(collect|direct)_targets$")
                        __qt56_internal_merge_libs(libs ${lib_target})
                    else()
                        __qt56_internal_merge_libs(libs "$<TARGET_LINKER_FILE:${lib_target}>")
                    endif()

                    get_target_property(target_rcc_objects "${lib_target}" _qt56_rcc_objects)
                    if(target_rcc_objects)
                        __qt56_internal_merge_libs(rcc_objects ${target_rcc_objects})
                    endif()
                ...
                if(operation STREQUAL "promote_3rd_party_global")
                    ...
                    _qt56_internal_should_not_promote_package_target_to_global(
                        "${lib_target_unaliased}" should_not_promote)
                    if(is_imported AND NOT should_not_promote)
                        _qt56_internal_promote_3rd_party_target_to_global(
            ...
            elseif("${lib_target}" MATCHES "^(Qt|${QT_CMAKE_EXPORT_NAMESPACE})::(.*)")
                ...
                if(NOT operation MATCHES "^(collect|direct)_targets$")
                    ...
                    __qt56_internal_merge_libs(libs "${final_lib_name_to_merge}")
                endif()
            endif()
        endforeach()
        __qt56_internal_memoize_values_in_dict("${target}" "${dict_name}" "libs" "${libs}")
        __qt56_internal_memoize_values_in_dict("${target}" "${dict_name}"
...
function(__qt56_internal_print_missing_dependency_target_warning target dep)
...
function(__qt56_internal_collect_all_target_dependencies target out_var)
    set(dep_targets "")
                ...
                if(TARGET "${lib}")
                    ...
                    foreach(lib_target IN LISTS lib_walked_targets)
                        if(NOT TARGET "${lib_target}")
                            __qt56_internal_print_missing_dependency_target_warning(${target}
                                ${lib_target})
                            continue()
                        endif()
