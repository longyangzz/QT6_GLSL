...
function(_qt56_internal_copy_file_if_different_command out_var src_file dst_file)
    ...
    if(CMAKE_HOST_WIN32 AND CMAKE_VERSION VERSION_LESS 3.23)
        set(${out_var} "${CMAKE_COMMAND}"
            ...
            -P "${_qt56_6_config_cmake_dir}/QtCopyFileIfDifferent.cmake"
            PARENT_SCOPE
        )
    else()
        set(${out_var} "${CMAKE_COMMAND}"
...
function(_qt56_internal_check_depfile_support out_var)
...
function(__qt56_internal_check_path_points_to_cmake_dir result path)
...
function(__qt56_internal_reverse_prefix_path_from_cmake_dir result cmake_path)
    string(TOUPPER "${QT_CMAKE_EXPORT_NAMESPACE}" export_namespace_upper)
...
function(__qt56_internal_get_possible_cmake_dirs out_paths prefix_path)
    set(${out_paths} "")
...
# with "/lib/cmake".
function(__qt56_internal_collect_additional_prefix_paths out_var prefixes_var)
    if(DEFINED "${out_var}")
        return()
    endif()
    ...
    foreach(additional_path IN LISTS additional_packages_prefixes)
        ...
        __qt56_internal_check_path_points_to_cmake_dir(is_path_to_cmake "${additional_path}")
        if(is_path_to_cmake)
            list(APPEND additional_packages_prefix_paths "${additional_path}")
        else()
            __qt56_internal_get_possible_cmake_dirs(additional_cmake_dirs "${additional_path}")
...
function(__qt56_internal_collect_additional_module_paths)
    if(__qt56_additional_module_paths_set)
        return()
    endif()
    foreach(prefix_path IN LISTS QT_ADDITIONAL_PACKAGES_PREFIX_PATH)
        __qt56_internal_check_path_points_to_cmake_dir(is_path_to_cmake "${prefix_path}")
        if(is_path_to_cmake)
    ...
    set(__qt56_additional_module_paths_set TRUE PARENT_SCOPE)
endfunction()

# Take a list of prefix paths ending with "/lib/cmake", and return a list of absolute paths with
# "/lib/cmake" removed.
function(__qt56_internal_prefix_paths_to_roots out_var prefix_paths)
    set(result "")
    foreach(path IN LISTS prefix_paths)
        __qt56_internal_reverse_prefix_path_from_cmake_dir(path "${path}")
...
function(_qt56_internal_collect_buildsystem_targets result dir)
    cmake_parse_arguments(arg "" "" "EXCLUDE;INCLUDE" ${ARGN})

    if(NOT _qt56_internal_collect_buildsystem_targets_inner)
        set(${result} "")
        set(_qt56_internal_collect_buildsystem_targets_inner TRUE)
    endif()
    ...
    foreach(subdir IN LISTS subdirs)
        _qt56_internal_collect_buildsystem_targets(${result} "${subdir}" ${forward_args})
    endforeach()
...
# Dependencies must then be added with _qt56_internal_add_phony_target_dependencies.
#
...
# _qt56_internal_add_phony_target_dependencies) and create the target at the end of the top-level
# directory scope.
#
...
#
# Example:
#     _qt56_internal_add_phony_target(update_translations
#          WARNING_VARIABLE QT_NO_GLOBAL_LUPDATE_TARGET_CREATED_WARNING
#     )
#     _qt56_internal_add_phony_target_dependencies(update_translations
#          narf_lupdate_zort_lupdate
#     )
#
function(_qt56_internal_add_phony_target target)
    ...
    if(CMAKE_GENERATOR MATCHES "^Visual Studio ")
        ...
        get_property(already_deferred GLOBAL PROPERTY _qt56_target_${target}_creation_deferred)
        if(NOT already_deferred)
            cmake_language(EVAL CODE
                "cmake_language(DEFER DIRECTORY \"${CMAKE_SOURCE_DIR}\" CALL _qt56_internal_add_phony_target_deferred \"${target}\")"
            )
            if(DEFINED arg_TARGET_CREATED_HOOK)
                set_property(GLOBAL
                    PROPERTY _qt56_target_${target}_creation_hook ${arg_TARGET_CREATED_HOOK}
                )
            endif()
        endif()
        set_property(GLOBAL APPEND PROPERTY _qt56_target_${target}_creation_deferred ON)
    else()
...
# _qt56_internal_add_phony_target. See the docstring at _qt56_internal_add_phony_target for
# more details.
function(_qt56_internal_add_phony_target_dependencies target)
    set(dependencies ${ARGN})
    if(CMAKE_GENERATOR MATCHES "^Visual Studio ")
        set_property(GLOBAL APPEND PROPERTY _qt56_target_${target}_dependencies ${dependencies})
...
function(_qt56_internal_add_phony_target_deferred target)
    get_property(target_dependencies GLOBAL PROPERTY _qt56_target_${target}_dependencies)
    set(target_commands "")
    ...
    get_property(creation_hook GLOBAL PROPERTY _qt56_target_${target}_creation_hook)
    if(creation_hook)
...
function(_qt56_internal_check_multiple_inclusion targets_not_defined targets)
    set(targets_defined "")
...
function(_qt56_internal_create_versionless_alias_targets targets install_namespace)
...
function(_qt56_internal_create_versionless_targets targets install_namespace)
    ...
    set(known_qt56_exported_properties
        MODULE_PLUGIN_TYPES
    ...
    set(known_qt56_exported_properties_interface_allowed
        _qt56_config_module_name
        _qt56_is_public_module
        _qt56_module_has_headers
        _qt56_module_has_private_headers
        _qt56_module_has_public_headers
        _qt56_module_has_qpa_headers
        _qt56_module_has_rhi_headers
        _qt56_module_include_name
        _qt56_module_interface_name
        _qt56_package_name
        _qt56_package_version
        _qt56_private_module_target_name
        _qt56_sbom_spdx_id
        _qt56_sbom_spdx_repo_document_namespace
        _qt56_sbom_spdx_relative_installed_repo_document_path
        _qt56_sbom_spdx_repo_project_name_lowercase
    )
    ...
    foreach(target IN LISTS targets)
        ...
        if(NOT "${type}" STREQUAL "INTERFACE_LIBRARY")
            ...
            foreach(property IN LISTS known_qt56_exported_properties)
                get_target_property(exported_property_value
        ...
        foreach(property IN LISTS known_qt56_exported_properties_interface_allowed)
            get_target_property(exported_property_value
                ${install_namespace}::${target} ${property})
            if(exported_property_value)
                set_property(TARGET Qt::${target} APPEND PROPERTY
        ...
        set_property(TARGET Qt::${target} PROPERTY _qt56_is_versionless_target TRUE)
    endforeach()
endfunction()
...
function(_qt56_internal_validate_all_args_are_parsed prefix)
...
function(_qt56_internal_path_join out_var)
    string(JOIN "/" path ${ARGN})
    set(${out_var} ${path} PARENT_SCOPE)
endfunction()

# _qt56_internal_qt56_remove_args can remove arguments from an existing list of function
...
#   endfunction()
#
function(_qt56_internal_remove_args out_var)
...
function(__qt56_internal_handle_find_all_qt56_module_packages out_var)
    set(opt_args "")
    set(single_args "")
    set(multi_args
        COMPONENTS
    ...
    _qt56_internal_validate_all_args_are_parsed(arg)
    ...
    if("ALL_QT_MODULES" IN_LIST components)
        list(FIND components "ALL_QT_MODULES" all_qt56_modules_index)
        list(REMOVE_AT components ${all_qt56_modules_index})
        ...
        set(all_qt56_modules "")

        foreach(json_module IN LISTS json_modules)
            get_filename_component(module_name "${json_module}" NAME_WE)
            list(APPEND all_qt56_modules ${module_name})
        endforeach()

        if(all_qt56_modules)
            list(INSERT components "${all_qt56_modules_index}" ${all_qt56_modules})
        endif()
    endif()
...
function(_qt56_internal_append_to_target_property_without_duplicates target property_name)
...
function(_qt56_internal_append_to_cmake_property_without_duplicates property_name)
...
#
# This is somewhat the opposite of _qt56_internal_remove_args.
#
# Parameters:
...
function(_qt56_internal_forward_function_args)
    set(opt_args
        FORWARD_APPEND
    )
    set(single_args
...
# Synopsis
#
#   _qt56_internal_relative_path(<path-var>
...
function(_qt56_internal_relative_path path_var)
    set(option_args "")
    set(single_args
        BASE_DIRECTORY
        OUTPUT_VARIABLE
