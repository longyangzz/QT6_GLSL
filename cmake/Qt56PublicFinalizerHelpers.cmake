...
function(__qt56_internal_check_finalizer_mode target out_var finalizer)
    set(option_args "")
    ...
    get_target_property(value ${target} _qt56_${finalizer}_finalizer_mode)
    if("${value}" STREQUAL "value-NOTFOUND")
        __qt56_internal_enable_finalizer_mode(${target} ${finalizer} "${arg_DEFAULT_VALUE}")
        set(value "${arg_DEFAULT_VALUE}")
    endif()
    set(${out_var} "${value}" PARENT_SCOPE)
endfunction()

function(__qt56_internal_enable_finalizer_mode target finalizer enabled)
    if(enabled)
        set(enabled "TRUE")
    else()
        set(enabled "FALSE")
    endif()
    set_property(TARGET "${target}" PROPERTY _qt56_${finalizer}_finalizer_mode "${enabled}")
endfunction()
