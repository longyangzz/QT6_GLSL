...
function(__qt56_internal_plugin_get_plugin_type
         plugin_target
...
function(__qt56_internal_plugin_has_class_name plugin_target out_var)
    set(plugin_target_versioned "${QT_CMAKE_EXPORT_NAMESPACE}::${plugin_target}")
...
function(__qt56_internal_get_static_plugin_condition_genex
...
# (not finalizer mode).
function(__qt56_internal_get_plugin_condition_regular_mode plugin_condition out_var)
    set(not_finalizer_mode "$<NOT:$<BOOL:$<TARGET_PROPERTY:_qt56_static_plugins_finalizer_mode>>>")
...
function(__qt56_internal_add_static_plugin_linkage plugin_target qt_module_target)
    __qt56_internal_get_static_plugin_condition_genex("${plugin_target}" plugin_condition)
    __qt56_internal_get_plugin_condition_regular_mode("${plugin_condition}" full_plugin_condition)
...
function(__qt56_internal_get_plugin_import_macro plugin_target out_var)
...
function(__qt56_internal_get_plugin_include_prelude out_var)
...
function(__qt56_internal_add_static_plugin_import_macro
        plugin_target
        qt_module_target
        qt_module_unprefixed)

    __qt56_internal_get_static_plugin_init_target_name("${plugin_target}" plugin_init_target)
    set(plugin_init_target_namespaced "${QT_CMAKE_EXPORT_NAMESPACE}::${plugin_init_target}")

    __qt56_internal_propagate_object_library(
        "${QT_CMAKE_EXPORT_NAMESPACE}::${plugin_target}"
        "${plugin_init_target_namespaced}"
        EXTRA_CONDITIONS
            "$<NOT:$<BOOL:$<TARGET_PROPERTY:_qt56_static_plugins_finalizer_mode>>>"
    )
endfunction()

# Get target name of object library which is used to initialize a qt plugin.
function(__qt56_internal_get_static_plugin_init_target_name plugin_target out_var)
...
function(__qt56_internal_add_static_plugin_init_object_library
        plugin_target
        out_var_plugin_init_target)

    __qt56_internal_get_plugin_import_macro(${plugin_target} import_macro)
    __qt56_internal_get_plugin_include_prelude(include_prelude)
    set(import_content "${include_prelude}${import_macro}")

    string(MAKE_C_IDENTIFIER "${plugin_target}" plugin_target_identifier)
    set(generated_qt56_plugin_file_name
        "${CMAKE_CURRENT_BINARY_DIR}/${plugin_target_identifier}_init.cpp")

    file(GENERATE
        OUTPUT "${generated_qt56_plugin_file_name}"
    ...
    set_source_files_properties("${generated_qt56_plugin_file_name}" PROPERTIES GENERATED TRUE)

    __qt56_internal_get_static_plugin_init_target_name("${plugin_target}" plugin_init_target)

    qt6_add_library("${plugin_init_target}" OBJECT "${generated_qt56_plugin_file_name}")
    ...
    set_property(TARGET ${plugin_init_target} PROPERTY _is_qt56_plugin_init_target TRUE)
    set_property(TARGET ${plugin_init_target} APPEND PROPERTY
        EXPORT_PROPERTIES _is_qt56_plugin_init_target
    )
...
function(__qt56_internal_collect_plugin_libraries plugin_targets out_var)
    set(plugins_to_link "")
    ...
    foreach(plugin_target ${plugin_targets})
        ...
        __qt56_internal_get_static_plugin_condition_genex(
            "${plugin_target}"
            plugin_condition)
...
function(__qt56_internal_collect_plugin_init_libraries plugin_targets out_var)
...
function(__qt56_internal_collect_plugin_library_files target plugin_targets out_var)
    set(library_files "")
...
# Plugins are collected by inspecting the _qt56_plugins property found on any dependency Qt target.
function(__qt56_internal_collect_plugin_targets_from_dependencies target out_var)
    set(dep_targets "")
    ...
    foreach(dep_target ${dep_targets})
        get_target_property(plugins ${dep_target} _qt56_plugins)
        if(plugins)
    ...
    __qt56_internal_collect_plugin_targets_from_dependencies_of_plugins(
...
function(__qt56_internal_collect_plugin_targets_from_dependencies_of_plugins target out_var)
    set(assigned_plugins_overall "")

    get_target_property(assigned_qt56_plugins "${target}" QT_PLUGINS)

    if(assigned_qt56_plugins)
        foreach(assigned_qt56_plugin ${assigned_qt56_plugins})
            if(TARGET "${assigned_qt56_plugin}")
                list(APPEND assigned_plugins_overall "${assigned_qt56_plugin}")
            endif()
        endforeach()
    endif()

    get_target_property(assigned_qt56_plugins_by_type "${target}" _qt56_plugins_by_type)

    if(assigned_qt56_plugins_by_type)
        foreach(assigned_qt56_plugin ${assigned_qt56_plugins_by_type})
            if(TARGET "${assigned_qt56_plugin}")
                list(APPEND assigned_plugins_overall "${assigned_qt56_plugin}")
            endif()
        endforeach()
...
#                      __qt56_internal_collect_plugin_targets_from_dependencies.
function(__qt56_internal_generate_plugin_deployment_info target)
    ...
    get_target_property(marked_for_deployment ${target} _qt56_marked_for_deployment)
    if(NOT marked_for_deployment)
        return()
    endif()

    __qt56_internal_collect_plugin_library_files(${target} "${plugin_targets}" plugins_files)
...
function(__qt56_internal_apply_plugin_imports_finalizer_mode target)
    # Process a target only once.
    get_target_property(processed ${target} _qt56_plugin_finalizer_imports_processed)
    if(processed)
        return()
    endif()

    __qt56_internal_generate_plugin_deployment_info(${target}
    ...
    __qt56_internal_collect_plugin_init_libraries("${plugin_targets}" init_libraries)
    __qt56_internal_collect_plugin_libraries("${plugin_targets}" plugin_libraries)

    target_link_libraries(${target} PRIVATE "${plugin_libraries}" "${init_libraries}")

    set_target_properties(${target} PROPERTIES _qt56_plugin_finalizer_imports_processed TRUE)
endfunction()
...
macro(__qt56_internal_include_plugin_packages target)
    set(__qt56_${target}_plugin_module_target "${QT_CMAKE_EXPORT_NAMESPACE}::${target}")
    set(__qt56_${target}_plugins "")
    ...
    get_target_property(_aliased_target ${__qt56_${target}_plugin_module_target} ALIASED_TARGET)
    if(_aliased_target)
        set(__qt56_${target}_plugin_module_target ${_aliased_target})
    endif()

    # Include all PluginConfig.cmake files and update the _qt56_plugins and QT_PLUGINS property of
    ...
    file(GLOB __qt56_${target}_plugin_config_files
        "${CMAKE_CURRENT_LIST_DIR}/${QT_CMAKE_EXPORT_NAMESPACE}*PluginConfig.cmake")
    foreach(__qt56_${target}_plugin_config_file ${__qt56_${target}_plugin_config_files})
        string(REGEX REPLACE
            "^.*/${QT_CMAKE_EXPORT_NAMESPACE}(.*Plugin)Config.cmake$"
            "\\1"
            __qt56_${target}_qt56_plugin "${__qt56_${target}_plugin_config_file}")
        include("${__qt56_${target}_plugin_config_file}")
        if(TARGET "${QT_CMAKE_EXPORT_NAMESPACE}::${__qt56_${target}_qt56_plugin}")
            list(APPEND __qt56_${target}_plugins ${__qt56_${target}_qt56_plugin})
        endif()
    endforeach()
    set_property(TARGET ${__qt56_${target}_plugin_module_target}
                 PROPERTY _qt56_plugins ${__qt56_${target}_plugins})

    # TODO: Deprecated. Remove in Qt 7.
    set_property(TARGET ${__qt56_${target}_plugin_module_target}
                 PROPERTY QT_PLUGINS ${__qt56_${target}_plugins})

    get_target_property(__qt56_${target}_have_added_plugins_already
        ${__qt56_${target}_plugin_module_target} __qt56_internal_plugins_added)
    if(__qt56_${target}_have_added_plugins_already)
        return()
    endif()

    foreach(plugin_target ${__qt56_${target}_plugins})
        __qt56_internal_plugin_get_plugin_type("${plugin_target}" __has_plugin_type __plugin_type)
        if(NOT __has_plugin_type)
            continue()
        endif()

        __qt56_internal_plugin_has_class_name("${plugin_target}" __has_class_name)
        ...
        if(type STREQUAL STATIC_LIBRARY)
            __qt56_internal_add_static_plugin_linkage(
                "${plugin_target}" "${__qt56_${target}_plugin_module_target}")
            __qt56_internal_add_static_plugin_import_macro(
                "${plugin_target}" ${__qt56_${target}_plugin_module_target} "${target}")
        endif()
    endforeach()

    set_target_properties(
        ${__qt56_${target}_plugin_module_target} PROPERTIES __qt56_internal_plugins_added TRUE)
endmacro()
...
macro(__qt56_internal_include_qml_plugin_packages)
    ...
    set(__qt56_qml_plugins_config_file_list "")
    set(__qt56_qml_plugins_glob_prefixes "${CMAKE_CURRENT_LIST_DIR}")
    ...
    if(QT_ADDITIONAL_QML_PLUGIN_GLOB_PREFIXES)
        foreach(__qt56_qml_plugin_glob_prefix IN LISTS QT_ADDITIONAL_QML_PLUGIN_GLOB_PREFIXES)
            if(__qt56_qml_plugin_glob_prefix)
                list(APPEND __qt56_qml_plugins_glob_prefixes "${__qt56_qml_plugin_glob_prefix}")
            endif()
        endforeach()
    endif()

    list(REMOVE_DUPLICATES __qt56_qml_plugins_glob_prefixes)

    foreach(__qt56_qml_plugin_glob_prefix IN LISTS __qt56_qml_plugins_glob_prefixes)
        file(GLOB __qt56_qml_plugins_glob_config_file_list
            "${__qt56_qml_plugin_glob_prefix}/QmlPlugins/${INSTALL_CMAKE_NAMESPACE}*Config.cmake")
        if(__qt56_qml_plugins_glob_config_file_list)
            list(APPEND __qt56_qml_plugins_config_file_list ${__qt56_qml_plugins_glob_config_file_list})
        endif()
    endforeach()

    if (__qt56_qml_plugins_config_file_list AND NOT QT_SKIP_AUTO_QML_PLUGIN_INCLUSION)
        # First round of inclusions ensure all qml plugin targets are brought into scope.
        foreach(__qt56_qml_plugin_config_file ${__qt56_qml_plugins_config_file_list})
            include(${__qt56_qml_plugin_config_file})
        ...
        foreach(__qt56_qml_plugin_config_file ${__qt56_qml_plugins_config_file_list})
            ...
            if(${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE)
                string(APPEND ${CMAKE_FIND_PACKAGE_NAME}_NOT_FOUND_MESSAGE
                    "\nThe message was set in ${__qt56_qml_plugin_config_file} ")
