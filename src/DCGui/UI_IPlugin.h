...
#ifndef DCGUI_IPLUGIN_H
#define DCGUI_IPLUGIN_H
...
namespace DCGui
{
	...
	public:
		...
		void SetSlotsObject(QObject* object)
		{
			pObject = object;
		}
		...
		void CreateMenuBySqlite(QString cfgFile)
		{
			...
			//! 从配置文件中获取模块名称
			QSqlQuery query;
			QString sql = "SELECT value from Setting where key = 'ModuleName'";
			...
			while (query.next())
			{
				ModuleName = query.value(0).toString();
			}
