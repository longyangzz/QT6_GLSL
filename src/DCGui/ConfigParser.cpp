...
bool ConfigParser::ConnectDatabaseWithoutPassword(QString& databaseName)
{
	...
	m_dbconn = QSqlDatabase::addDatabase("QSQLITE", "DCLWGP");
	m_dbconn.setDatabaseName(databaseName);

	//打开数据库
	if (!m_dbconn.open())
	{
		qDebug() << m_dbconn.lastError();
...
QString ConfigParser::GetLicenseState()
{
	GetQssFileName();
	QSqlQuery query(m_dbconn);
	QString value = "";
	QString sql = QString("select Value from Global where key= '%1' ").arg("License");
	query.exec(sql);
	//如果存在值则返回该值，不存在，则返回空
	while (query.next())
	{
...
MenuVec ConfigParser::GetMenuVec()
{
	...
	Menu currentMenu;
	QSqlQuery query(m_dbconn);
	QString sql = "select * from Menu order by ShowIndex";
	...
	while (query.next())
	{
		...
		for (int i = 0; i != recordNum; ++i)
		{
			...
			else if (fieldName.toUpper() == "TOOLBARPOS")
			{
				currentMenu.toolbarPos = query.value(fieldIndex).toInt();
			}
...
QString ConfigParser::GetQssFileName()
{
	QSqlQuery query(m_dbconn);

	QString sql = QString("select Value from Global where Key = '%1' ").arg("Qss");
